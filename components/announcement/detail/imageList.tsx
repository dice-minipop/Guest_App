import { Image } from 'expo-image';
import { useState } from 'react';
import { Dimensions, Text, View } from 'react-native';
import Carousel from 'react-native-reanimated-carousel';

import { AnnouncementDetailComponentProps } from '@/types/announcement';

export default function AnnouncementImageListComponent({ data }: AnnouncementDetailComponentProps) {
  const width = Dimensions.get('screen').width;

  const [currentIndex, setCurrentIndex] = useState<number>(0);

  return (
    <View className="relative">
      <Carousel
        width={width}
        height={210}
        loop={true}
        data={data.imageUrls}
        snapEnabled={true}
        pagingEnabled={true}
        autoPlay={false}
        onSnapToItem={setCurrentIndex}
        renderItem={({ item }) => <Image source={item} style={{ width: width, height: 210 }} />}
      />

      <View className="absolute bottom-[20px] right-[20px] bg-black/50 rounded-full px-[9px] py-[1.5px]">
        <Text className="BTN1 text-white">
          {currentIndex + 1} / {data.imageUrls.length}
        </Text>
      </View>
    </View>
  );
}
