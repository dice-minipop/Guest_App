...
export default function ChatItemComponent({ data, spaceName }: ChatItemComponentProps) {
  if (data.type === 'NOTICE') {
    return (
      <View className="bg-white mt-[24px] rounded-lg p-[16px] mx-[20px]">
        <Text className="CAP1 text-deep_gray text-center">
          {`'${spaceName}' 담당자님과의 쪽지가 시작되었습니다. 불필요한 비방과 부적절한 언행은 제재 대상이 될 수 있습니다.`}
        </Text>
      </View>
    );
    ...
    return (
      <View className="self-end mr-[20px]">
        <Text className="CAP1 text-medium_gray text-right">{formatChatTime(data.createdAt)}</Text>
    ...
    return (
      <View className="ml-[20px]">
        <Text className="CAP1 text-medium_gray">
          {data.senderName} · 호스트 {formatChatTime(data.createdAt)}
        </Text>
